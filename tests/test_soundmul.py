...
class Test_soundmul(unittest.TestCase):
    ...
    def test_len(self): # aqui voy a comprobar que la longitud del buffer del objeto antiguo es igual que la del nuevo buffer
        self.assertEqual(len(s.buffer),len(s2.buffer))

    def test_values(self):

        # Verificar que cada valor del buffer se haya multiplicado correctamente
